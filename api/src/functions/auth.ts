  ...
  const forgotPasswordOptions: DbAuthHandlerOptions['forgotPassword'] = {
    ...
    handler: async (user) => {
      const res = await sendEmail({
        ...
        text: `You can reset your password by clicking here: ${
          process.env.DEPLOY_URL || process.env.DOMAIN_URL
        }/reset-password?resetToken=${user.resetToken}`,
        html: `
        <p>Click the link below to reset your password.</p>
        <a href="${
          process.env.DEPLOY_URL || process.env.DOMAIN_URL
        }/reset-password?resetToken=${user.resetToken}">Reset your password</a>
      `,
      })
      if (res && res.accepted) {
        createAudit({
